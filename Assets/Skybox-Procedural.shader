...
Shader "Skybox/ProceduralLowPoly" {
Properties {
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		#if SKYBOX_SUNDISK == SKYBOX_SUNDISK_HQ
			OUT.vertex 			= -v.vertex;
			//OUT.center = mul(unity_ObjectToWorld, v.vertex);
			OUT.center = v.vertex;
		#elif SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
		...
		void geom(triangle v2f input[3], inout TriangleStream<v2f> OutputStream)
		{
			...
			#if SKYBOX_SUNDISK == SKYBOX_SUNDISK_HQ
				center = (input[1].center.xyz + input[0].center.xyz + input[2].center.xyz) / 3;
			#endif
			half3 ground = (input[1].groundColor.xyz + input[0].groundColor.xyz + input[2].groundColor.xyz) / 3;
				half3 sky = (input[1].skyColor.xyz + input[0].skyColor.xyz + input[2].skyColor.xyz) / 3;
			//float3 view = normalize(_WorldSpaceCameraPos.xyz - center);
			for (int i = 0; i < 3; i++)
			{
				temp = input[i];
				temp.center = center;
				temp.groundColor = ground;
				temp.skyColor = sky;
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			half3 rayc = normalize(mul((float3x3)unity_ObjectToWorld, IN.center));
			half yc = ray.y / SKY_GROUND_THRESHOLD;
			//half y = normalize(mul((float3x3)unity_ObjectToWorld, IN.center)).y / SKY_GROUND_THRESHOLD;
		#elif SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
			...
			col = lerp(IN.skyColor, IN.groundColor, saturate(yc));
			//col = half3(y, y, y);
			//col = lerp(IN.skyColor, IN.groundColor, saturate(normalize(mul((float3x3)unity_ObjectToWorld, IN.center)).y));
			//col = lerp(IN.skyColor, IN.groundColor, 0);
